...
/**
 * Returns the elements of the given [map] as a list ordered by ascending keys.
 */
fun <K : Comparable<K>, V> mapEntriesAscending(map: Map<K, V>): List<Pair<K, V>> =
    map.toSortedMap().map { it.key to it.value }
...
class Parsertest {
    ...
    @Test
    fun `test correct file parsing`() {
        val parser = MapParser(config = TestStrings.ExampleString)
        val smapRes = parser.createMap()
